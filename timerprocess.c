...
static void LinkedListInitialization (linked_list_t* );
static void LinkedListInsetion (linked_list_t* , timer_data*);
static void LinkedListDeletion (linked_list_t* , link_t*,unsigned long );
static void LinkedListTraverse (linked_list_t* , void (*callback) (timer_data*));
static void LinkedListTraverseReverse (linked_list_t*,void (*callback) (timer_data*));
static void LinkedListDeleteSequenceNumber (linked_list_t* , int, unsigned long );
static void LinkedListFree (linked_list_t* );
static void PrintLinkedList (timer_data*);
//end of function definitions


/* The below Method copies fields from source structure into
...
/* The below Method initializes the linked list by putting zeros
 into the pointers containing the first and last links of the linked
 list. */

static void LinkedListInitialization (linked_list_t * list){
    list->first = list->last = 0;
}

/* The below Method adds a new link to the end of the linked
 list. It allocates memory for it. The contents of the link are
 copied from "data". */

static void LinkedListInsetion (linked_list_t * list, timer_data *data){
...
/* The below Method is used to remove specified node from Delta Time List while
 simultaneously accounting for passed_time */

static void LinkedListDeletion (linked_list_t * list, link_t * link, unsigned long passed_time){
    link_t * prev;
    link_t * next;
...
/* The function is used to traverse the entire Delta Time List */

static void LinkedListTraverse (linked_list_t * list, void (*callback) (timer_data*)){
    link_t * link;
...
/* The function is used to traverse the entire Delta Time List in the reverse order */
static void LinkedListTraverseReverse (linked_list_t *list,void (*callback) (timer_data*)){
    link_t * link;
...
/* The function is used to delete the packet with the given sequence number from the Delta Time List */
static void LinkedListDeleteSequenceNumber (linked_list_t * list, int sequence_number, unsigned long passed_time){
    link_t * link;
    
    for (link = list->first; link; link = link->next) {
        if (link->data->sequence_number== sequence_number) {
            LinkedListDeletion (list, link, passed_time);
        }
    }
}


/*The function is used to Free the list's memory. */

static void LinkedListFree (linked_list_t * list) {
    link_t * link;
    link_t * next;
...
static void PrintLinkedList (timer_data *data){
...
int main () {
    ...
    while(1) {
        ...
        printf("Waiting...\n");
        if(list.first == NULL) {
            printf("Currently the Delta Timer Empty...\n");
            if(select(FD_SETSIZE+1, &readset, NULL, NULL, NULL) < 0 ) {
                perror("In the line 335");
                exit (EXIT_FAILURE);
            }
            else {
                if(FD_ISSET(sock, &readset)) {
                    if(recvfrom(sock, timer_request, MAX_MES_LEN, 0, (struct sockaddr *)&tcpd_name, &namelen) < 0) {
                        ...
                        printf("[TCPD_M2 to TIMER]  Received the timer packet with sequence_number: %d\n",timer_request->sequence_number);
                        if(timer_request->type == 0){
                            struct timer_data * packet = calloc (1,sizeof(struct timer_data));
                            printf("\nStarting the timer for the packet with Sequence Number: %d\t and Expire Time: %.2f\n",timer_request->sequence_number,timer_request->time);
                            copy(timer_request,packet);
                            LinkedListInsetion(&list, packet);
                        }
                        else if (timer_request->type == 1) {
                            perror("Warning!!! Empty List...Therefore cannot Delete...\n");
                            //exit(7);
                        }
                        else {
                ...
                if(FD_ISSET(sock, &readset)) {
                    if(recvfrom(sock, timer_request, MAX_MES_LEN, 0, (struct sockaddr *)&tcpd_name, &namelen) < 0) {
                        ...
                        if(timer_request->type == 0){
                            ...
                            printf("\n Canceling the time for packet with sequence number sequence Number: %d\n",timer_request->sequence_number);
                            LinkedListDeleteSequenceNumber(&list, timer_request->sequence_number, wait_time - tv.tv_sec);
                        }
                        else {
                            perror("Undefined Packet Type");
                            exit(6);
                    ...
                    printf("\n TIMER has has expire for the Packet with sequence Number: %d\n",list.first->data->sequence_number);
                    LinkedListTraverse (&list,PrintLinkedList);
                    exp_seq_num = list.first->data->sequence_number ;
                    printf("[TIMER to TCPD_M2] sending the expire packet with sequence_number: %d\n",exp_seq_num);
                    ...
                    LinkedListDeleteSequenceNumber(&list,list.first->data->sequence_number, wait_time);
                }
            }
        }     
    }
